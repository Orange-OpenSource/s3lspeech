...
URLS = {
    ...
    # LibriSpeech finetuning https://github.com/facebookresearch/libri-light/tree/main/data_preparation
    'LIBRILIGHT': "https://dl.fbaipublicfiles.com/librilight/data/librispeech_finetuning.tgz",

    # Noise
    'RIRS_NOISES': "https://www.openslr.org/resources/28/rirs_noises.zip",
    'DNS_NOISE': "https://dns3public.blob.core.windows.net/dns3archive/datasets/datasets.noise.tar.bz2",
}
...
def get_librilight(data_root, data_set="librispeech_finetuning"):
    """
    Downloads LibriLight data and process it into WAV files, 
    generating a manifest in JSON format

    Args:
        data_root: local folder
        data_sets: LibriSpeech finetuning dataset 
    Returns:

    [1] https://github.com/facebookresearch/libri-light/tree/main/data_preparation
    """
    librilight = os.path.join(data_root, "librispeech_finetuning")
        
    logging.info("\n\nWorking on: {0}".format(data_set))
    filepath = os.path.join(data_root, data_set + ".tgz")
    logging.info("Getting {0}".format(data_set))
    __maybe_download_file(filepath, 'LIBRILIGHT')
    logging.info("Extracting {0}".format(data_set))
    __extract_file(filepath, data_root)
    logging.info("Processing {0}".format(data_set))
    __process_data(
        librilight,
        os.path.join(librilight, '10h' + "-processed"),
        os.path.join(librilight, 'librilight10h' + ".json")
    )
    __process_data(
        os.path.join(librilight, '1h'),
        os.path.join(librilight, '1h' + "-processed"),
        os.path.join(librilight, 'librilight1h' + ".json")
    )
    __process_data(
        os.path.join(librilight, '1h', '0'),
        os.path.join(librilight, '10min' + "-processed"),
        os.path.join(librilight, 'librilight10min' + ".json")
    )
    logging.info('Done!')

def load_librilight(data_root, data_sets, max_batch_length) -> DataLoader:
    """
    ...
    Args:
        data_root: local folder
        data_sets: LibriLight dataset
    Returns:
        dataloader: pytorch dataloader with dynamic batching
    """
    return load_librispeech(
        data_root, data_sets, max_batch_length,
        dataset_name="librispeech_finetuning")
